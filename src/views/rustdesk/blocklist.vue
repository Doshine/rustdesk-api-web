<template>
  <div class="yj-settings-panel" v-loading="form.loading">
    <header class="yj-settings-panel__header">
      <h2 class="yj-settings-panel__title">封禁列表 <span class="yj-settings-panel__code">BLOCK_LIST</span></h2>
      <p class="yj-settings-panel__desc">被封禁的对象无法通过中继服务器建立或继续保持连接。</p>
    </header>

    <div class="yj-settings-group">
      <div class="yj-settings-row yj-settings-row--column">
        <div class="yj-settings-row__main">
          <div class="yj-settings-row__name">当前封禁（{{ form.list.length }}）</div>
        </div>
      ...
      <div class="yj-settings-row">
        <div class="yj-settings-row__main">
          <div class="yj-settings-row__name">封禁操作</div>
    ...
    <el-dialog v-model="form.form_visible" :title="form.form_type === 'add' ? '添加封禁' : '删除封禁'" width="480px">
      <el-form label-width="72px">
        <el-form-item label="ID / IP">
          <el-input v-model="form.form_input"></el-input>
          <div class="yj-dialog-hint">
            多个条目以 | 分割<span v-if="form.form_type==='delete'">，全部删除填 <strong>all</strong></span>
          </div>
        </el-form-item>
      </el-form>
      <template #footer>
...
<script setup>
  ...
  const submit = async () => {
    if (!form.form_input.trim()) {
      ElMessage.error(T('ParamRequired', { param: 'ID / IP' }))
      return
    }
    // 高风险操作：保存前确认影响范围与回滚方式
    const ok = await confirmRisk(form.form_type === 'add' ? 'blocklist_add' : 'blocklist_remove')
    if (!ok) {
      return
    }
